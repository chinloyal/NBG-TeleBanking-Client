	...
	public RegistrationView() {
		...
		contentPane.setLayout(new FormLayout(
				new ColumnSpec[] { FormSpecs.RELATED_GAP_COLSPEC, FormSpecs.DEFAULT_COLSPEC,
						...
						FormSpecs.RELATED_GAP_COLSPEC, ColumnSpec.decode("max(156dlu;default):grow"),
						FormSpecs.RELATED_GAP_COLSPEC, FormSpecs.DEFAULT_COLSPEC, FormSpecs.RELATED_GAP_COLSPEC,
						FormSpecs.DEFAULT_COLSPEC, FormSpecs.RELATED_GAP_COLSPEC, FormSpecs.DEFAULT_COLSPEC,
	...
	public void actionPerformed(ActionEvent event) {
		if (event.getSource().equals(rdbtnCustomer)) {// radio button event
			...
			if (fileChooser.showOpenDialog(this) == JFileChooser.APPROVE_OPTION) {
				...
				uploadedImageName = renameImage(uploadedImageName) + "-" + (new Date()).getTime() + "."
						+ FilenameUtils.getExtension(uploadedImage.getName());
				...
				if (user.getType().equals("") || user.getType().equals("customer")) {
					...
					if (uploadedImage != null) {
						...
						if (!extensions.matcher(uploadedImage.getName()).matches()) {
							JOptionPane.showMessageDialog(null, "Unacceptable file format.", "Error",
